...
for event in cumulated_data:
    ...
    if "FirerPosition" in keys:
        if "ParentProjectile" not in keys:  # count shots
            shot_count += 1
    ...
    if "Event" in keys:
        ...
        if (
                event["data_dict"]["Event"] not in ["Deflected", "MPKilled", 'VehicleCriticalDammaged']
                and "<NULL-object>" in event["data_dict"].values()
