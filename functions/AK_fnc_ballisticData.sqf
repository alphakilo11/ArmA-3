...
*/  
 ...
 _projectile addEventHandler ["HitPart", {  
	...
	_hitEntity addEventHandler ["Dammaged", {  
	 ...
	 if (isNull _shooter == false) then { 
	  ...
	  if (_hitPoint in ["hithull","hitltrack","hitrtrack","hitengine","hitturret","hitgun"]) then { 
	   ...
	   _dammageLogEntry = format ['{"Function": "AK_fnc_ballisticData", "Event Handler": "Vehicle Critical Dammaged", "Unit": "%1", "Selection": "%2", "Damage": %3, "HitIndex": %4, "HitPoint": "%5", "Shooter": "%6", "Projectile": "%7", "diag_tickTime": %8, "Vehicle Damage ["hithull","hitltrack","hitrtrack","hitengine","hitturret","hitgun"]": %9 }', _unit, _selection, _damage, _hitIndex, _hitPoint, _shooter, _projectile, diag_tickTime, _criticalHitPointStatus];   
  ...
  _firedMessage = format ['{"Function": "AK_fnc_ballisticData", "Event Handler": "Fired", "Shooter":"%1", "Shooter Type":"%2", "Shooter Position":%3, "Shooter Velocity":%4, "Projectile":"%5", "Muzzle Velocity":%6, "Weapon":"%7", "Ammo":"%8", "Shooter Side":"%9", "_shotTime": %10}', _gunner, typeOf _unit, getPosASL _gunner, velocity _unit, _projectile,  _muzzleVelocity, _weapon, _ammo, side _gunner, _shotTime]; 
