...
AK_fnc_traceProjectile = {
  ...
  _firedMessage = format ['{"Function": "AK_fnc_ballisticData", "Event Handler": "Fired", "Shooter":"%1", "Shooter Type":"%2", "Shooter Position":%3, "Shooter Velocity":%4, "Projectile":"%5", "Muzzle Velocity":%6, "Weapon":"%7", "Ammo":"%8", "Shooter Side":"%9", "tickTime": %10}', _gunner, typeOf _unit, getPosASL _gunner, velocity _unit, _projectile,  _muzzleVelocity, _weapon, _ammo, side _gunner, _shotTime]; 
  diag_log _firedMessage; 
  if (AK_switch_traceProjectileDebug == true) then {systemChat _firedMessage;}; 
  ...
  _ins = lineIntersectsSurfaces [  
   ...
   if (count _ins != 0) then { 
	...
	if (AK_switch_traceProjectileDebug == true) then {hintSilent format ['%1 is the Initial Target', _initialTarget];};   
   }; 
  ...
  if (isNil "AK_var_traceProjectileData") then {AK_var_traceProjectileData = []};  
