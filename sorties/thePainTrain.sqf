...
{
	[[([2023, 6, 21] + (missionStart select [3, 2])), time] call BIS_fnc_calculateDateTime, true, true] call BIS_fnc_setDate;
} remoteExec ["call", 2];// set longest day
...
AK_handle_dynVis = [{
	[{
		[diag_fps, true, [10, 30, 40]] remoteExec ["AK_fnc_dynAdjustVisibility", 2];
	}] remoteExec ["call", -2];
