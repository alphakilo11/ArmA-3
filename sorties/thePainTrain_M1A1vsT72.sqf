...
// spawn defenders
[_anchorPosition, _size, true, configFile >> "CfgGroups" >> "Indep" >> "rhsgref_faction_chdkz_g" >> "rhs_group_indp_ins_g_72" >> "RHS_T72baSection", independent, _defenderNumberOfGroups] spawn AK_fnc_populateMap; 
// enable dynamic visiblity adjustment
AK_handle_dynVis = [{[{[diag_fps, true, [10, 20, 26]] remoteExec ["AK_fnc_dynAdjustVisibility", 2];}] remoteExec ["call", -2];}, 10] call CBA_fnc_addperFrameHandler;
...
AK_handle_attackers = [     
    { 
        ...
        if ([AK_var_attackers] call AK_fnc_countUnits <= _attackerCount) then { 
            _group = [_ingressPosition, west, configFile >> "CfgGroups" >> "West" >> "rhs_faction_usmc_wd" >> "rhs_group_nato_usmc_wd_m1a1" >> "RHS_M1A1FEP_wd_Section"] call BIS_fnc_spawnGroup; //RANDOM_CONFIG_CLASS(RANDOM_CONFIG_CLASS(RANDOM_CONFIG_CLASS(configFile >> "cfgGroups" >> "west")))  
